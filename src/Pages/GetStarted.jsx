  ...
  return (
    <Layout>
      ...
      <Box display="flex" fontSize={"2xl"} mt="5" fontWeight={"semibold"}>
        ...
        <Text color={"pink.300"} ml="2" fontWeight={"bold"}>
          CStylfer
        </Text>
      </Box>
      <Box px={"10"} mt="5">
        <Text>
          No need to do. Just choose what theme you wanna try on the side bar,
          play with it. Get CSS code that you need, copy it to your Web Project
        </Text>
      ...
      <Text fontSize={"2xl"} mt={3} fontWeight={"semibold"}>
        About The Designs theme
      </Text>
      {designs?.map((item, i) => {
        return (
          <Box mt={2}>
            <Text fontSize={"xl"} fontWeight={"semibold"} color={"blue.300"}>
              {item.title}
            </Text>
            <Text ml={2}>{item.desc}</Text>
            {item.link && (
              <Text ml={2}>
                Reference link for this design is
